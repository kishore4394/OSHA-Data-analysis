...
dt = dataset
dt = data.table(dataset)
Employer_count_injury = dt[,sum(count), by = dt$Employer]
Employer_count_injury = Employer_count_injury[Employer_count_injury$V1 > 13]
Employer_count = ggplot(Employer_count_injury, aes(x=dataset, y=V1))+geom_bar(stat = "identity")+ ggtitle("Maximum injuries vs Employers")+xlab("City")+ylab("Count")+theme(axis.title.x = element_blank(), axis.text.x = element_text(angle = 45, hjust = 1))
...
# groupby and count of particular values in a dataset 
#t = data.frame(table(dataset$Industrynames))
#t = arrange(t,desc(Freq))
#t[grep("postal service",t$Var1),] like function in r

# values of the employer are changed using like function
#Amputation_dataset[grep("postal service",Amputation_dataset$Employer),]$Employer = "United State Postal Service"
#dataset = read.csv("preprocessed_data.csv")
#dataset$Employer = as.character(dataset$Employer)
#dataset[grep("postal service", dataset$Employer),]$Employer = "United State Postal Service"
#dataset[grep("Usps", dataset$Employer),]$Employer = "United State Postal Service"
#dataset[grep("usps", dataset$Employer),]$Employer = "United State Postal Service"
